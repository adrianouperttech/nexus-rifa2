...
export const buyPlan = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(501).json({ message: "Funcionalidade de comprar plano desativada." });
  } catch (error) {
    next(error);
  }
};
...
export const updateSettings = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // res.json({ success: true, user: updated });
     res.status(501).json({ message: "Funcionalidade de atualizar configurações desativada." });
  } catch (error) {
    next(error);
  }
};
...
export const createRaffle = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(501).json({ message: "Funcionalidade de criar rifa desativada." });
  } catch (error) {
    next(error);
  }
};
...
export const getRaffles = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(501).json({ message: "Funcionalidade de obter rifas desativada." });
  } catch (error) {
    next(error);
  }
};
